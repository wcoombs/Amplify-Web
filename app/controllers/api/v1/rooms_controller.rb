  ...
  module V1
    class RoomsController < Api::V1::BaseController
      ...
      def show
        ...
        @playlist_data = playlist_data(@host.room.songs.where(song_status_id: song_statuses[:votable]), voter).sort_by{|s| -s[:total_score]}
        respond_to do |format|
      ...
      def next_song
        ...
        songs = room.songs
        played = songs.find_by(song_status_id: song_statuses[:currently_playing])
        curr = songs.find_by(song_status_id: song_statuses[:up_next])
        up_next = songs.left_outer_joins(:votes).where(song_status_id: song_statuses[:votable]).group(:id).order('sum(votes.score) desc').first
        if played
          Song.destroy(played.id)
        end
        if curr
          curr.update(song_status_id: song_statuses[:currently_playing])
        end
        if up_next
          up_next.update(song_status_id: song_statuses[:up_next])
        end
